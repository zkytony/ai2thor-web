...
def launch_controller(config):
    controller = Controller(scene=config["scene_name"],
                            agentMode=config.get("agentMode", "default"),
                            width=config.get("width", 300),
                            height=config.get("height", 300),
                            visibilityDistance=config.get("visibilityDistance", 5.0),
                            fieldOfView=config.get("fov", 120),
                            gridSize=config.get("gridSize", 0.25),
                            renderDepthImage=config.get("renderDepth", True),
                            renderClassImage=config.get("renderClass", True),
                            renderObjectImage=config.get("renderObject", True),
                            continuous=config.get("continuous", True),
                            snapToGrid=config.get("snapToGrid", False))
    return controller
